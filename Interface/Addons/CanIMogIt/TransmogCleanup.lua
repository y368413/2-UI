...
local function isItemIgnored(itemLink)
	...
	return TransmogCleanupDB.ignoredItems[id]
end
			...
			if link then
				...
				if status ~= nil then
					if status and status > 0 then -- check if Can I Mog It checks the itemList
						if TransmogCleanupDB.filters.onuse or not hasOnUseEffect then -- not (not TransmogCleanupDB.filters.onuse and hasOnUseEffect)
							if ilvl <= TransmogCleanupDB.filters.ilvl then
								if TransmogCleanupDB.filters.quality[quality] then
									if TransmogCleanupDB.filters.bind[bind] then
										if TransmogCleanupDB.filters.learned[status] then
								...
								--dbg(link, "ilvl", ilvl, TransmogCleanupDB.filters.ilvl, ilvl < TransmogCleanupDB.filters.ilvl)
							end
						else
...
local function isSafeModeEnabled()
	return TransmogCleanupDB.sellMode.safe
end

local function isVerboseModeEnabled()
	return TransmogCleanupDB.sellMode.verbose
end
...
local function sellItems()
	...
	if itemsSold > 0 then
		print(("你卖了 %d 件幻化物品，赚了 %s "):format(itemsSold, GetCoinTextureString(itemsSoldValue), itemsSold))
	else
		print("哥，全卖完了啊.")
	end
end
...
local function onIgnoredCheckedChange(self, ...)
	if IsControlKeyDown() then
		...
		TransmogCleanupDB.ignoredItems[self.itemId] = self:GetChecked()
	end
end
...
local function onFilterCheckedChange(self, ...)
	if self.type=="onuse" then
		TransmogCleanupDB.filters[self.type] = self:GetChecked()
	else
		TransmogCleanupDB.filters[self.type][self.id] = self:GetChecked()
	end
	updateItemList(sellWindow)
end

local function onSellModeCheckedChange(self, ...)
	TransmogCleanupDB.sellMode[self.type] = self:GetChecked()
end
...
local function ilvlSliderValueChanged(self, value)
	...
	TransmogCleanupDB.filters.ilvl = newValue
...
local function createSellWindow()
	...
	frame:SetWidth(MerchantFrame:GetWidth()/2)
...
local function updateSellSettings(sellWindow)
	for i =1,#sellWindow.qualityCBs do
		sellWindow.qualityCBs[i]:SetChecked(TransmogCleanupDB.filters.quality[i])
	end

	for i =1,#sellWindow.bindCBs do
		sellWindow.bindCBs[i]:SetChecked(TransmogCleanupDB.filters.bind[i])
	end

	for i =1,#sellWindow.learnedCBs do
		sellWindow.learnedCBs[i]:SetChecked(TransmogCleanupDB.filters.learned[i])
	end

	sellWindow.onUseCB:SetChecked(TransmogCleanupDB.filters.onuse)

	sellWindow.ilvlSlider:SetValue(TransmogCleanupDB.filters.ilvl)

	sellWindow.safeModeCB:SetChecked(TransmogCleanupDB.sellMode.safe)

	sellWindow.verboseModeCB:SetChecked(TransmogCleanupDB.sellMode.verbose)
end
...
local function createMerchantButton()
	if not merchantButton then
		...
		merchantButton:SetPoint("BOTTOMLEFT", MerchantFrame, "BOTTOMLEFT", 2, 2)
...
function events:ADDON_LOADED()
		...
		TransmogCleanupDB = TransmogCleanupDB
		local db = TransmogCleanupDB
