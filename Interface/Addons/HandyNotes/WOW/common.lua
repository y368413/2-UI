...
local Class = WorldOfWarcraft.Class
local Group = WorldOfWarcraft.Group

local Collectible = WorldOfWarcraft.node.Collectible
local NPC = WorldOfWarcraft.node.NPC
local Rare = WorldOfWarcraft.node.Rare
...
WorldOfWarcraft.expansion = 1

-------------------------------------------------------------------------------
----------------------------------- GROUPS ------------------------------------
-------------------------------------------------------------------------------

WorldOfWarcraft.groups.DRAGONRACE = Group('dragonrace', 1100022, {
    defaults = WorldOfWarcraft.GROUP_HIDDEN,
    type = WorldOfWarcraft.group_types.EXPANSION,
...
WorldOfWarcraft.groups.SAFARI = Group('safari', 4048818, {defaults = WorldOfWarcraft.GROUP_HIDDEN})
WorldOfWarcraft.groups.CRAZYFORCATS = Group('crazyforcats', 656579,
    {defaults = WorldOfWarcraft.GROUP_HIDDEN})
...
WorldOfWarcraft.node.AnniversaryRare = Class('AnniversaryRare', Rare, {
...
WorldOfWarcraft.node.AnniversaryNPC = Class('AnniversaryNPC', NPC, {
    sublabel = L['anniversary_note'],
    IsEnabled = function(self)
        if not IsAnniversaryEvent() then return false end
        return Rare.IsEnabled(self)
...
local Safari = Class('Safari', Collectible,
    {icon = 'paw_g', group = WorldOfWarcraft.groups.SAFARI})

WorldOfWarcraft.node.Safari = {
...
WorldOfWarcraft.node.CrazyForCats = Class('CrazyForCats', Collectible,
    {icon = 656579, group = WorldOfWarcraft.groups.CRAZYFORCATS})

-------------------------------------------------------------------------------
----------------------------- ZUL'GURUB (10.0.7) ------------------------------
-------------------------------------------------------------------------------

WorldOfWarcraft.groups.SECRETS_OF_ZULGURUB = WorldOfWarcraft.Group('secrets_of_zulgurub', 132528, {
    defaults = WorldOfWarcraft.GROUP_HIDDEN,
    type = WorldOfWarcraft.group_types.EXPANSION
})
...
local Dragonrace = Class('DragonRace', Collectible,
    {icon = 1100022, group = WorldOfWarcraft.groups.DRAGONRACE})
...
function Dragonrace.getters:note()
    if self.normal then
        local silver = WorldOfWarcraft.color.Silver
        local gold = WorldOfWarcraft.color.Gold
...
WorldOfWarcraft.node.Dragonrace = Dragonrace
...
hooksecurefunc(AreaPOIPinMixin, 'TryShowTooltip', function(self)
    if not WorldOfWarcraft.groups.DRAGONRACE:IsEnabled() or
        not DRAGONRACE_POI[self.areaPoiID] then return end
    local mapID = self:GetMap().mapID
    local group = WorldOfWarcraft.groups.DRAGONRACE

    if not WorldOfWarcraft.maps[mapID] or not group:GetDisplay(mapID) then return end

    local node = WorldOfWarcraft.maps[mapID].nodes[DRAGONRACE_POI[self.areaPoiID]]
    if not node then return end

    --GameTooltip:AddLine(' ')
    GameTooltip:AddLine(WorldOfWarcraft.RenderLinks(node.sublabel, true), 1, 1, 1)
    if WorldOfWarcraft:GetOpt('show_notes') then
        --GameTooltip:AddLine(' ')
        GameTooltip:AddLine(WorldOfWarcraft.RenderLinks(node.note), 1, 1, 1, true)
    end
    if WorldOfWarcraft:GetOpt('show_loot') then
        --GameTooltip:AddLine(' ')
...
hooksecurefunc(VignettePinMixin, 'DisplayNormalTooltip', function(self)
    if not WorldOfWarcraft.groups.DRAGONRACE:IsEnabled() or self.vignetteID ~= 5104 then
        return
    end
    ...
    local node = WorldOfWarcraft.maps[mapID].nodes[HandyNotes:getCoord(x, y)]
    if not node then return end

    GameTooltip:SetText(WorldOfWarcraft.RenderLinks(node.label, true))
    GameTooltip:AddLine(WorldOfWarcraft.RenderLinks(node.sublabel, true), 1, 1, 1)
    if WorldOfWarcraft:GetOpt('show_notes') then
        --GameTooltip:AddLine(' ')
        GameTooltip:AddLine(WorldOfWarcraft.RenderLinks(node.note), 1, 1, 1, true)
...
local ScavengerPool = WorldOfWarcraft.Class('scavengerpool', WorldOfWarcraft.node.Collectible,
    {icon = 132765})

function ScavengerPool:Initialize(criteria)
    WorldOfWarcraft.node.Collectible.Initialize(self)
    self.rewards = {Achievement({id = 1257, criteria = criteria})}
end

WorldOfWarcraft.node.ScavengerPool = ScavengerPool
