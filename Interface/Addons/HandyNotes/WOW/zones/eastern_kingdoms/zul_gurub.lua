...
local Item = WorldOfWarcraft.reward.Item
local Quest = WorldOfWarcraft.reward.Quest
local Recipe = WorldOfWarcraft.reward.Recipe
local Section = WorldOfWarcraft.reward.Section
local Spacer = WorldOfWarcraft.reward.Spacer
local Spell = WorldOfWarcraft.reward.Spell
local Toy = WorldOfWarcraft.reward.Toy
local Transmog = WorldOfWarcraft.reward.Transmog

local Circle = WorldOfWarcraft.poi.Circle
local Path = WorldOfWarcraft.poi.Path
...
map.nodes[29324855] = Collectible({
    ...
    group = WorldOfWarcraft.groups.SECRETS_OF_ZULGURUB,
    pois = {
        Path({
            color = 'Blue',
            points = {
...
local Rinwosho = Class('Rinwosho', Collectible, {
    ...
    quest = 74576, -- Restored Hakkari Bijou
    group = WorldOfWarcraft.groups.SECRETS_OF_ZULGURUB,
    requires = WorldOfWarcraft.requirement.Item(203737), -- Restored Hakkari Bijou
    rewards = {
        Achievement({id = 17366}), -- Relics of a Falled Empire
...
function BigBagOBijous.getters:note()
    local function count(id)
        if WorldOfWarcraft.PlayerHasItem(id) then
            return WorldOfWarcraft.status.Green(C_Item.GetItemCount(id, true))
        else
            return WorldOfWarcraft.status.Red(0)
        end
    end
...
map.nodes[54773273] = Collectible({
    ...
    -- LuaFormatter on
    requires = WorldOfWarcraft.requirement.Spell(401577), -- Mudskunk Aroma
    questDeps = 74576, -- Restored Hakkari Bijou
    group = WorldOfWarcraft.groups.SECRETS_OF_ZULGURUB,
    rewards = {
...
map.nodes[56624826] = Collectible({
    ...
    group = WorldOfWarcraft.groups.SECRETS_OF_ZULGURUB,
    requires = WorldOfWarcraft.requirement.Profession(171),
    rewards = {
...
local BonePile = Class('BonePile', Collectible, {
    icon = 133731,
    requires = WorldOfWarcraft.requirement.Spell(401721), -- Succumbed to Madness
    questDeps = 74576, -- Restored Hakkari Bijou
    group = WorldOfWarcraft.groups.SECRETS_OF_ZULGURUB,
    IsEnabled = function() return true end,
    rewards = {
...
function BonePile.getters:note()
    local function status(id, itemsNeed)
        ...
        return WorldOfWarcraft.PlayerHasItem(id, itemsNeed) and
                   WorldOfWarcraft.status.Green(itemsHave .. '/' .. itemsNeed) or
                   WorldOfWarcraft.status.Red(itemsHave .. '/' .. itemsNeed)
    end
