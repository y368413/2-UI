...
local Spacer = TheWarWithin.reward.Spacer

-------------------------------------------------------------------------------

local IsleOfDorn = TheWarWithin.maps[2248]
local RingingDeeps = TheWarWithin.maps[2214]
local Hallowfall = TheWarWithin.maps[2215]
local AzjKahet = TheWarWithin.maps[2255]

local Dornogal = TheWarWithin.maps[2339]
-------------------------------------------------------------------------------
--------------------------------- DRAGONRACES ---------------------------------
-------------------------------------------------------------------------------

local Dragonrace = Class('DragonRace', Collectible,
    {icon = 1100022, group = TheWarWithin.groups.DRAGONRACE})
...
function Dragonrace.getters:note()
    local Silver = TheWarWithin.color.Silver
    local Gold = TheWarWithin.color.Gold
    local note = L['dr_target_time']
    local txt = L['dr_your_target_time']
    for _, race in ipairs(DRAGONRIDING_RACE_TYPES) do
        if self[race.type] then
            local label = race.label
            local sTime = TheWarWithin.color.Red('??')
            if self[race.type][2] then
                sTime = Silver(self[race.type][2])
            end
            local gTime = TheWarWithin.color.Red('??')
