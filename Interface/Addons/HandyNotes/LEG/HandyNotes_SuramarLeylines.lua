...
local get_point_info_by_coord = function(uiMapId, coord)
    return get_point_info(SuramarLeylines.points[uiMapId] and SuramarLeylines.points[uiMapId][coord])
end
...
local handle_tooltip_by_coord = function(tooltip, uiMapId, coord)
    return handle_tooltip(tooltip, SuramarLeylines.points[uiMapId] and SuramarLeylines.points[uiMapId][coord])
end
...
local function hideNode(button, uiMapId, coord)
    SuramarLeylines.hidden[uiMapId][coord] = true
    HL:Refresh()
end
...
do
    ...
    local function iter(t, prestate)
        ...
        while state do -- Have we reached the end of this zone?
            if value and SuramarLeylines:ShouldShow(value) then
                ...
                return state, nil, icon, SuramarLeylines.db.icon_scale, SuramarLeylines.db.icon_alpha
            end
    ...
    function HLHandler:GetNodes2(uiMapId, minimap)
        currentLevel = level
        currentZone = uiMapId
        return iter, SuramarLeylines.points[uiMapId], nil
    end
    function SuramarLeylines:ShouldShow(point)
        if point.entrance and not SuramarLeylines.db.entrances then
            return false
        end
        if point.level and point.level ~= currentLevel then
            return false
        end
        if SuramarLeylines.db.complete then
            return true
        end
        if point.achievement then
            if point.criteria then
...
function HL:OnInitialize()
    -- Set up our database
    self.db = LibStub("AceDB-3.0"):New("HandyNotes_SuramarLeylinesDB", SuramarLeylines.defaults)
...
SuramarLeylines.options = {
    type = "group",
    name = "HandyNotes_SuramarLeylines",
    get = function(info) return SuramarLeylines.db[info[#info]] end,
    set = function(info, v)
        SuramarLeylines.db[info[#info]] = v
        SuramarLeylines.HL:SendMessage("HandyNotes_NotifyUpdate", "HandyNotes_SuramarLeylines")
    end,
    args = {
        icon = {
            type = "group",
