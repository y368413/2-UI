...
function info:GuildPanel_Update()
	local scrollFrame = OrzUIGuildInfobarScrollFrame
	local usedHeight = 0
...
local function sortRosters(a, b)
	if a and b then
		if OrzUISetDB["GuildSortOrder"] then
			return a[OrzUISetDB["GuildSortBy"]] < b[OrzUISetDB["GuildSortBy"]]
		else
			return a[OrzUISetDB["GuildSortBy"]] > b[OrzUISetDB["GuildSortBy"]]
		end
	end
...
local function sortHeaderOnClick(self)
	OrzUISetDB["GuildSortBy"] = self.index
	OrzUISetDB["GuildSortOrder"] = not OrzUISetDB["GuildSortOrder"]
...
function info:GuildPanel_Init()
	...
	infoFrame = CreateFrame("Frame", "OrzUIGuildInfobar", info)
	...
	local scrollFrame = CreateFrame("ScrollFrame", "OrzUIGuildInfobarScrollFrame", infoFrame, "HybridScrollFrameTemplate")
...
info.onEnter = function()
	...
	if OrzUIFriendsFrame and OrzUIFriendsFrame:IsShown() then
		OrzUIFriendsFrame:Hide()
	end
