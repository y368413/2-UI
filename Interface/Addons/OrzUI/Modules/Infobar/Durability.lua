...
info.onMouseUp = function(self, btn)
	if btn == "RightButton" then
		OrzUISetDB["RepairType"] = mod(OrzUISetDB["RepairType"] + 1, 3)
		self:onEnter()
	else
...
info.onEnter = function(self)
	...
	GameTooltip:AddDoubleLine(" ", U["Auto Repair"]..": "..repairlist[OrzUISetDB["RepairType"]].." ", 1,1,1, .6,.8,1)
	GameTooltip:Show()
end
...
function autoRepair(override)
	...
	if canRepair and repairAllCost > 0 then
		if (not override) and OrzUISetDB["RepairType"] == 1 and IsInGuild() and CanGuildBankRepair() and GetGuildBankWithdrawMoney() >= repairAllCost then
			RepairAllItems(true)
		else
...
local function merchantShow()
	if not OrzUISetDB["Help"]["AutoRepair"] then
		HelpTip:Show(MerchantFrame, autoRepairInfo)
	end

	if IsShiftKeyDown() or OrzUISetDB["RepairType"] == 0 or not CanMerchantRepair() then return end
	autoRepair()
