...
--[[MISC.preload = {}
function MISC:RegisterPreload(name, func)
...
function MISC:OnLogin()
	...
	MISC:HandleOrzUITitle()
	MISC:ToggleAddOnProfiler()
	MISC:MDEnhance()
	MISC:LootEx()		-- 拾取增强
	...
	-- Auto chatBubbles
	if OrzUISetDB["AutoBubbles"] then
...
function MISC:VehicleSeatMover()
	local frame = CreateFrame("Frame", "OrzUIVehicleSeatMover", UIParent)
...
function MISC:UIWidgetFrameMover()
	local frame1 = CreateFrame("Frame", "OrzUIUIWidgetMover", UIParent)
	...
	local frame2 = CreateFrame("Frame", "OrzUIUIWidgetPowerBarMover", UIParent)
...
-- Drag AltPowerbar
do
	local mover = CreateFrame("Frame", "OrzUIAltBarMover", PlayerPowerBarAlt)
	...
	PlayerPowerBarAlt:HookScript("OnEnter", function(self)
		if not OrzUISetDB["Help"]["AltPower"] then
...
function MISC:JerryWay()
	...
	SlashCmdList["ORZUI_JERRY_WAY"] = function(msg)
...
function MISC:BaudErrorFrameHelpTip()
	...
	hooksecurefunc(count, "SetText", function(_, text)
		if not OrzUISetDB["Help"]["BaudError"] then
			text = tonumber(text)
...
function MISC:ToggleAddOnProfiler()
	local function CheckState()
		return OrzUISetDB["AddOnProfiler"]
	end
	...
	M.ReskinCheck(bu)
	M.CreateFS(bu, 14, U["CPU Usage"], "info", "LEFT", 30, 0)
	bu:SetChecked(CheckState())
	bu:SetScript("OnClick", function()
		OrzUISetDB["AddOnProfiler"] = bu:GetChecked()
	end)
end
...
do
	M:RegisterEvent("PLAYER_CHOICE_UPDATE", function()
...
-- ReloadUI Made by Sharpedge_Gaming ## Version: v.2.9
local function CreateGameMenuButton()
    ...
    GameMenuFrame:HookScript("OnShow", function()
        button:SetPoint("TOP", GameMenuFrame, "TOP", 0, 6) 
    end)
end
