...
function MISC:HandleOrzUITitle()
	-- Square OrzUI logo texture
	local function replaceIconString(self, text)
		if not text then text = self:GetText() end
		if not text or text == "" then return end

		if strfind(text, "OrzUI") or strfind(text, "AngryKeystones") or strfind(text, "Baganator") or strfind(text, "CollectionShop") or strfind(text, "LiteBuff") or strfind(text, "BattlePetMaster") or strfind(text, "GarrisonMaster") or strfind(text, "GladiatorlosSACN") or strfind(text, "Skada") or strfind(text, "Syndicator") then
			...
			ProfessionsBookFrame:HookScript("OnShow", function()
				if not done then
					if InCombatLockdown() then
						...
						M.CreateMF(ProfessionsBookFrame)
					end
					done = true
				end
			end)
