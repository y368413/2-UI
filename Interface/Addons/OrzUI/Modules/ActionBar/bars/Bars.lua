...
function Bar:UpdateActionSize(name)
	local frame = _G["OrzUI_Action"..name]
...
function Bar:UpdateVisibility()
	for i = 1, 8 do
		local frame = _G["OrzUI_ActionBar"..i]
		if frame then
...
function Bar:CreateBars()
	Bar.headers = {}
	for index = 1, 8 do
		Bar.headers[index] = CreateFrame("Frame", "OrzUI_ActionBar"..index, UIParent, "SecureHandlerStateTemplate")
	end

	local BAR_DATA = {
		[1] = {page = 1, bindName = "ACTIONBUTTON", anchor = {"BOTTOM", UIParent, "BOTTOM", 0, 0}},
		[2] = {page = 6, bindName = "MULTIACTIONBAR1BUTTON", anchor = {"BOTTOM", _G.OrzUI_ActionBar1, "TOP", 0, -margin}},
		[3] = {page = 5, bindName = "MULTIACTIONBAR2BUTTON", anchor = {"LEFT", _G.OrzUI_ActionBar1, "TOPRIGHT", -margin, 0}},
		[4] = {page = 3, bindName = "MULTIACTIONBAR3BUTTON", anchor = {"RIGHT", _G.OrzUI_ActionBar1, "TOPLEFT", margin, 0}},
		[5] = {page = 4, bindName = "MULTIACTIONBAR4BUTTON", anchor = {"RIGHT", UIParent, "RIGHT", -1, 0}},
		[6] = {page = 13, bindName = "MULTIACTIONBAR5BUTTON", anchor = {"RIGHT", _G.OrzUI_ActionBar5, "LEFT", margin, 0}},
...
function Bar:OnLogin()
	...
	M:RegisterEvent("HOUSE_EDITOR_MODE_CHANGED", Bar.UpdateHousingState)

	if AdiButtonAuras then
		AdiButtonAuras:RegisterLAB("LibActionButton-1.0-OrzUI")
	end
end