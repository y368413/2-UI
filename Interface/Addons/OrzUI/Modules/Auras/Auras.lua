...
function A:UpdateHeader(header)
	...
	if header.filter == "HELPFUL" then
		...
		header:SetAttribute("weaponTemplate", format("OrzUIAuraTemplate%d", cfg.size))
	end
	...
	header:SetAttribute("template", format("OrzUIAuraTemplate%d", cfg.size))
...
function A:CreateAuraHeader(filter)
	local name = "OrzUIPlayerDebuffs"
	if filter == "HELPFUL" then name = "OrzUIPlayerBuffs" end
...
function A:RemoveSpellFromIgnoreList()
	if IsAltKeyDown() and IsControlKeyDown() and self.spellID and R.db["AuraWatchList"]["IgnoreSpells"][self.spellID] then
		...
		print(format(U["RemoveFromIgnoreList"], I.OrzUIString, self.spellID))
	end
end
