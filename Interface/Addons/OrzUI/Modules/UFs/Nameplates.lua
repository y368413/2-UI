...
function UF:UpdateClickableSize()
	...
	local uiScale = OrzUISetDB["UIScale"]
...
function UF:AddQuestIcon(self)
	...
	count:SetFont("Interface\\AddOns\\OrzUI\\Media\\Fonts\\Infinity.ttf", 16, "OUTLINE")
...
function UF:AddDungeonProgress(self)
	...
	self.progressText:SetFont("Interface\\AddOns\\OrzUI\\Media\\Fonts\\Pixel.ttf", 14, "OUTLINE")
end
...
function UF:CreatePlates()
	...
	self:SetScale(OrzUISetDB["UIScale"])
...
function UF:ResizePlayerPlate()
	...
	if plate then
		...
		if plate.Avada then
			local iconSize = (barWidth+2*R.mult - R.margin*5)/6
			for i = 1, 6 do
...
local function RefreshNameplateFilter(list, key)
	...
	for spellID in pairs(list) do
		local name = GetSpellName(spellID)
		if name then
			if OrzUISetDB[key][spellID] == nil then
				UF[key][spellID] = true
			end
		end
	end
