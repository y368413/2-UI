...
C_Timer.NewTicker(0.1, function()
    ...
    if ctrlCnt==45 then
        PlaySoundFile("Interface\\AddOns\\OrzUI\\Media\\Sounds\\Legendary.ogg", "Master")
...
CrazyCatLady:SetScript("OnEvent", function(self, event, ...) 
  if not R.db["Misc"]["CrazyCatLady"] then self:UnregisterAllEvents() return end
	if event == "PLAYER_DEAD" then PlaySound(15476) elseif event == "PLAYER_UNGHOST" then StopMusic() end
end)

--------------------------------------------------------------------------- m4xLegendary
local m4xLegendary = CreateFrame("Frame")
m4xLegendary:RegisterEvent("SHOW_LOOT_TOAST_LEGENDARY_LOOTED")
m4xLegendary:SetScript("OnEvent", function(self) PlaySoundFile("Interface\\AddOns\\OrzUI\\Media\\Sounds\\Legendary.ogg", "Master") end)
...
local PetHealthAlarm = CreateFrame("ScrollingMessageFrame", "PetHealthAlarm", UIParent)
PetHealthAlarm.Warned = false
PetHealthAlarm:RegisterEvent("PLAYER_LOGIN")
PetHealthAlarm:RegisterEvent("UNIT_HEALTH")
PetHealthAlarm:SetScript("OnEvent", function(self, event, Arg1, ...)
    if(event == "PLAYER_LOGIN") then
        PetHealthAlarm:SetWidth(450)
        PetHealthAlarm:SetHeight(200)
        PetHealthAlarm:SetPoint("CENTER", UIParent, "CENTER", 0, 120)
        PetHealthAlarm:SetFont("Interface\\AddOns\\OrzUI\\Media\\Fonts\\RedCircl.TTF",36,"THICKOUTLINE")
        PetHealthAlarm:SetShadowColor(0.00, 0.00, 0.00, 0.75)
        PetHealthAlarm:SetShadowOffset(3.00, -3.00)
        PetHealthAlarm:SetJustifyH("CENTER")
        PetHealthAlarm:SetMaxLines(2)
        PetHealthAlarm:SetTimeVisible(2)
        PetHealthAlarm:SetFadeDuration(1)
        return
    end
    if(event == "UNIT_HEALTH" and Arg1 == "pet") then
        if(floor((UnitHealth("pet") / UnitHealthMax("pet")) * 100) <= 35 and PetHealthAlarm.Warned == false) then
			PlaySoundFile("Interface\\AddOns\\OrzUI\\Media\\Sounds\\Beep.ogg")	
        ...
        if (UnitName("pet") ~= "Pacifist Rig") then
        	PetHealthAlarm:AddMessage("|cFFFF0000".."- CRITICAL PET HEALTH -", 1, 0, 0, nil, 5)  ---- How long until message fades " - " .. petName .. " needs healing!"
        	PetHealthAlarm.Warned = true
        end
            return
        end
        if(floor((UnitHealth("pet") / UnitHealthMax("pet")) * 100) > 35) then PetHealthAlarm.Warned = false return end
        return
    end
end)