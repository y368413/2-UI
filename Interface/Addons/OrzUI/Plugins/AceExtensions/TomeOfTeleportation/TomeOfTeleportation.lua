--## Author: Remeen  5.11.4  GameMenuFrame
local ST_Item = 1
local ST_Spell = 2
local ST_Dungeon = 3
local ST_Raid = 4
...
local function AddColourOption(text, optionName, parent, hasAlpha, previous)
    ...
    local p = previous
    p = AddSliderOption(text .. " 红", optionName .. "R", 0, 1, nil, parent, p, true, colourPanelWidth, changeCallback)
    local redTitle = p
    p = AddSliderOption(text .. " 绿", optionName .. "G", 0, 1, nil, parent, p, true, colourPanelWidth, changeCallback)
    local greenTitle = p
    p = AddSliderOption(text .. " 蓝", optionName .. "B", 0, 1, nil, parent, p, true, colourPanelWidth, changeCallback)
    local blueTitle = p
    if hasAlpha then
        p = AddSliderOption(text .. " 透明度", optionName .. "A", 0, 1, nil, parent, p, true)
    end
...
local function CreateSettings(panel)
    ...
    p = AddCheckOption("所有盟约炉石", "allCovenants",         scrollChild, p)
    ...
    p = AddCheckOption("仅当前地下城",     "seasonOnly",           scrollChild, p)
    ...
    p = AddCheckOption("在任何地方显示法术",    "showInWrongZone",      scrollChild, p)
    ...
    p = AddCheckOption("搜索隐藏物品",       "searchHidden",         scrollChild, p)
    p = AddCheckOption("显示鼠标提示",             "tooltip",         scrollChild, p)
    ...
    p = AddStringOption("边框纹理",         "edge",                                 scrollChild, p)
    p = AddSliderOption("边框大小",            "frameEdgeSize", 0, 50, 1,              scrollChild, p)
    ...
    --p = AddStringOption("按钮字体",          "buttonFont",                         scrollChild, p)
    p = AddStringOption("按钮背景",    "buttonBackground",                     scrollChild, p)
    p = AddStringOption("按钮边框",          "buttonEdge",                           scrollChild, p)
    p = AddSliderOption("按钮边框大小",     "buttonEdgeSize", 0, 50, 1,             scrollChild, p)
    p = AddSliderOption("按钮平铺大小",     "buttonTileSize", 1, 50, 1,             scrollChild, p)
    p = AddColourOption("可用颜色",         "readyColour",                          scrollChild, false, p)
    ...
    p = AddColourOption("德鲁伊形态颜色",    "druidFormColour",                      scrollChild, false, p)

    --p = AddSliderOption("快捷菜单大小",      "QuickMenuSize", 20, 100, 1,             scrollChild, p)
end

local ZoneLabels = {}
local SpellFrames = {}

local TextShow = "已知时显示"
local TextHide = "隐藏"
local TextAlways = "总是显示"
...
local function RefreshSpellFrame(spellFrame, spell, parent, previous, refreshFunction)
    ...
    if spell.isCustom then
        if not spellFrame.deleteButton then
            ...
            spellFrame.deleteButton:SetText("删除")
...
function TeleporterSettings_OnLoad()
	...
	TeleporterSettings.spellsPanel.name = "自定义传送法术"
...
local dataobj = ldb:NewDataObject("TomeTele", {
	...
	type = "data source", 
	icon = "Interface\\Addons\\OrzUI\\Media\\2UI",  --Interface\\Icons\\Spell_Arcane_TeleportDalaran  --Icons\\Spell_arcane_portalshattrath
	--text = "Teleport"
})
...
function Teleporter_OnEvent(self, event, ...)
	if event == "ADDON_LOADED" then
		local loadedAddon = ...
		if string.upper(loadedAddon) == string.upper("OrzUI") then
...
local function OnClickRemove(spell)
	local dialogText = "你确定要删除吗 " .. spell.spellName .. "?"
...
local function ShowSelectDestinationUI(dialog, isItem)
	...
	if name then
		...
		if isItem then
			dialogText = "找不到此ID的物品。"
		else
			dialogText = "找不到此ID的法术。"
		end
...
local function ShowAddItemUI(isItem)
	...
	if isItem then
		dialogText = "输入物品ID，你可以从这里获取它的信息：wowhead.com."
	else
...
local function CreateMainFrame()
	...
	if GetOption("showHelp") then
		...
		helpString:SetText( "点击传送,Ctrl+点击创建宏。" )
...
function TeleporterOpenFrame(isSearching)
	if UnitAffectingCombat("player") then
		print( "战斗中无法使用" .. TOMEOFTELEPORTATIONTITLE .. "。" )
		return
	end
	...
	if not IsEquippedItem ( item ) then
		if IsEquippableItem( item ) then 
			...
			if itemSlot == nil then
				print( "未识别的可添加物品类型: " .. itemEquipLoc )
				return
			end			
...
function dataobj:OnEnter()
    ...
    --GameTooltip:AddDoubleLine(ERR_QUEST_HAS_IN_PROGRESS,"|T".."Interface\\Addons\\OrzUI\\Media\\Modules\\MaoR-UI"..":32|t")
    --GameTooltip:AddLine(" ")
...
local SetMrbarMicromenu = {  
   ...
   -- { text = PETS, icon = 'Interface\\MINIMAP\\TRACKING\\StableMaster',  --broke
    ...
    { text = MINIMAP_MENU_SWITCHUF, icon = 'Interface\\Icons\\Spell_Holy_Crusade',
        func = function() SenduiCmd("/loadui"); end, notCheckable = true},
    ...
    { text = "  |cFFBF00FF2 UI"..MINIMAP_MENU_UISETTING.."|r", func = function() SenduiCmd("/orzui"); end, notCheckable = true},
    { text = "|cffff8800 --------------------------|r", isTitle = true, notCheckable = true  },
