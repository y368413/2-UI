...
G.DefaultSettings = {
	...
	Misc = {
		...
		EnhanceDressup = true,
		QuestTool = true,
		InfoStrLeft = "[spec][time][guild][friend][ping][fps][gold][dura][Bags]",
		InfoStrRight = "[zone]",
		InfoSize = 13,
		MaxAddOns = 21,
		MenuButton = true,
...
G.AccountSettings = {
	...
	ProfileIndex = {},
	ProfileNames = {},
	Help = {["GUIPanel"] = true, ["MinimapInfo"] = true, ["AltPower"] = true, ["ChatScroll"] = true, ["ChatSwitch"] = true, ["AutoRepair"] = true, ["QuickJoin"] = true,},
	CornerSpells = {},
	CustomTex = "",
	MajorSpells = {},
	SmoothAmount = .25,
...
loader:SetScript("OnEvent", function(self, _, addon)
	...
	if OrzUISetDB["NameplateFilter"] then
		if OrzUISetDB["NameplateFilter"][1] then
			if not OrzUISetDB["NameplateWhite"] then OrzUISetDB["NameplateWhite"] = {} end
			for spellID, value in pairs(OrzUISetDB["NameplateFilter"][1]) do
				OrzUISetDB["NameplateWhite"][spellID] = value
			end
		end
		if OrzUISetDB["NameplateFilter"][2] then
			if not OrzUISetDB["NameplateBlack"] then OrzUISetDB["NameplateBlack"] = {} end
			for spellID, value in pairs(OrzUISetDB["NameplateFilter"][2]) do
				OrzUISetDB["NameplateBlack"][spellID] = value
			end
		end
	end
	if OrzUISetDB["RaidAuraWatch"] then
		if not OrzUISetDB["RaidBuffsWhite"] then OrzUISetDB["RaidBuffsWhite"] = {} end
		for spellID in pairs(OrzUISetDB["RaidAuraWatch"]) do
			OrzUISetDB["RaidBuffsWhite"][spellID] = true
		end
	end
	-- Transfer old data END

	InitialSettings(G.AccountSettings, OrzUISetDB)
	if not next(OrzUIPerDB) then
		for i = 1, 5 do OrzUIPerDB[i] = {} end
	end

	if not OrzUISetDB["ProfileIndex"][I.MyFullName] then
		OrzUISetDB["ProfileIndex"][I.MyFullName] = 1
	end

	if OrzUISetDB["ProfileIndex"][I.MyFullName] == 1 then
		R.db = OrzUIDB
	else
		R.db = OrzUIPerDB[OrzUISetDB["ProfileIndex"][I.MyFullName] - 1]
	end
	...
	M:SetupUIScale(true)
	if OrzUISetDB["CustomTex"] ~= "" then
		I.normTex = "Interface\\"..OrzUISetDB["CustomTex"]
	else
		if not G.TextureList[OrzUISetDB["TexStyle"]] then
			OrzUISetDB["TexStyle"] = 2 -- reset value if not exists
		end
		I.normTex = G.TextureList[OrzUISetDB["TexStyle"]].texture
	end
...
local function toggleAvada()
	M:GetModule("UnitFrames"):Avada_Toggle()
end
...
local function updateSmoothingAmount()
	M:SetSmoothingAmount(OrzUISetDB["SmoothAmount"])
end
...
local function updateInfobarAnchor(self)
	...
	if not OrzUISetDB["DisableInfobars"] then
...
G.OptionList = {		-- type, key, value, name, horizon, horizon2, doubleline
	...
	[2] = {
		...
		--{3, "UFs", "PrivateSize", "PrivateAuras", nil, nil, {5, 30, 1}},
		...
		{1, "UFs", "RaidClickSetsTooltips", HeaderTag.."快捷施法技能鼠标提示", true, true},
	...
	[4] = {
		...
		{1, "Avada", "Enable", IsNew..HeaderTag..U["Enable ClassAuras"].."*", true, nil, toggleAvadaGUI, toggleAvada},
		...
		{1, "Nameplate", "PPPowerText", U["PlayerPlate PowerText"].."*", true, nil, nil, togglePlatePower},
	...
	[5] = {
		...
		{4, "Misc", "AutoRoll", IsNew..U["Auto Roll"], true, true, {format("|A:%s:%s|a", "lootroll-toast-icon-need-up", "16:16").." "..NEED, format("|A:%s:%s|a", "lootroll-toast-icon-pass-up", "16:16").." "..PASS, DISABLE}},
		...
		--{1, "Misc", "HideQuests", U["HideQuests"].."*", true, true},
		{},--blank
		...
		{1, "Misc", "HunterPetHelp", U["HunterPetHelp"], true},
	...
	[8] = {
		...
		{1, "Tooltip", "CursorHappy", "鼠标闪光(方便定位)".."*", true},
		{1, "Misc", "ItemLevel", HeaderTag..U["Show ItemLevel"], nil, nil, nil, nil, U["ItemLevelTip"]},
		{1, "Misc", "MissingStats", HeaderTag..U["Show MissingStats"], true},
		{1, "Misc", "GemNEnchant", U["Show GemNEnchant"].."*", true, true},
		{1, "Misc", "AzeriteTraits", U["Show AzeriteTraits"].."*"},
		{1, "Misc", "HideTalking", U["No Talking"], true},
		{1, "ACCOUNT", "AutoBubbles", U["AutoBubbles"], true, true},
		{1, "Misc", "HideBossEmote", U["HideBossEmote"].."*", nil, nil, nil, toggleBossEmote},
		{1, "Misc", "HideBossBanner", U["Hide Bossbanner"].."*", true, nil, nil, toggleBossBanner},
		--{1, "Misc", "InstantDelete", U["InstantDelete"].."*", true, true},
		...
		{1, "Misc", "BlockInvite", "|cffff0000"..U["BlockInvite"].."*", true, nil, nil, nil, U["BlockInviteTip"]},
		{1, "Misc", "BlockRequest", "|cffff0000"..U["BlockRequest"].."*", true, true, nil, nil, U["BlockRequestTip"]},
		...
		{1, "Misc", "QuickJoin", HeaderTag..U["EnhancedPremade"], nil, nil, nil, nil, U["EnhancedPremadeTip"]},
		{1, "Misc", "SingingSocket", IsNew..U["SingingSocket"], true, nil, nil, U["SingingSocketTip"]},
...
local function CheckUIOption(key, value, newValue)
	if key == "ACCOUNT" then
		if newValue ~= nil then
			OrzUISetDB[value] = newValue
		else
			return OrzUISetDB[value]
		end
	else
		if newValue ~= nil then
...
StaticPopupDialogs["RELOAD_ORZUI"] = {
...
local function OpenGUI()
	...
	-- Main Frame
	f = CreateFrame("Frame", "OrzUIGUI", UIParent)
	tinsert(UISpecialFrames, "OrzUIGUI")
	...
	SwitchUF:SetScript("OnClick", function() SenduiCmd("/loadui") f:Hide() end)
	...
	if not OrzUISetDB["Help"]["GUIPanel"] then
