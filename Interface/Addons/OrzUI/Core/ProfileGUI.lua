...
-- Static popups
StaticPopupDialogs["RESET_ORZUI"] = {
	text = U["Reset OrzUI Check"],
	button1 = YES,
	button2 = NO,
	OnAccept = function()
		OrzUIDB = {}
...
StaticPopupDialogs["RESET_ORZUI_HELPINFO"] = {
	text = U["Reset OrzUI Helpinfo"],
	button1 = YES,
	button2 = NO,
	OnAccept = function()
		wipe(OrzUISetDB["Help"])
	end,
	whileDead = 1,
}

StaticPopupDialogs["ORZUI_RESET_PROFILE"] = {
...
StaticPopupDialogs["ORZUI_APPLY_PROFILE"] = {
	text = U["Apply selected profile?"],
	button1 = YES,
	button2 = NO,
	OnAccept = function()
		OrzUISetDB["ProfileIndex"][myFullName] = G.currentProfile
		ReloadUI()
	end,
	whileDead = 1,
}

StaticPopupDialogs["ORZUI_DOWNLOAD_PROFILE"] = {
	text = U["Download selected profile?"],
	button1 = YES,
	button2 = NO,
	OnAccept = function()
		local profileIndex = OrzUISetDB["ProfileIndex"][myFullName]
		if G.currentProfile == 1 then
			OrzUIPerDB[profileIndex-1] = OrzUIDB
		elseif profileIndex == 1 then
			OrzUIDB = OrzUIPerDB[G.currentProfile-1]
		else
			OrzUIPerDB[profileIndex-1] = OrzUIPerDB[G.currentProfile-1]
		end
		ReloadUI()
	end,
	whileDead = 1,
...
StaticPopupDialogs["ORZUI_UPLOAD_PROFILE"] = {
	...
	OnAccept = function()
		if G.currentProfile == 1 then
			...
			OrzUIPerDB[G.currentProfile-1] = R.db
		end
	end,
	whileDead = 1,
}

StaticPopupDialogs["ORZUI_DELETE_UNIT_PROFILE"] = {
	text = "",
	button1 = YES,
	button2 = NO,
	OnAccept = function(self)
		local name, realm = strsplit("-", self.text.text_arg1)
		if OrzUISetDB["totalGold"][realm] and OrzUISetDB["totalGold"][realm][name] then
			OrzUISetDB["totalGold"][realm][name] = nil
		end
		OrzUISetDB["ProfileIndex"][self.text.text_arg1] = nil
	end,
...
function G:Reset_OnClick()
	StaticPopup_Show("ORZUI_RESET_PROFILE")
end

function G:Apply_OnClick()
	G.currentProfile = self:GetParent().index
	StaticPopup_Show("ORZUI_APPLY_PROFILE")
end

function G:Download_OnClick()
	G.currentProfile = self:GetParent().index
	StaticPopup_Show("ORZUI_DOWNLOAD_PROFILE")
end

function G:Upload_OnClick()
	G.currentProfile = self:GetParent().index
	StaticPopup_Show("ORZUI_UPLOAD_PROFILE")
end

function G:GetClassFromGoldInfo(name, realm)
	local class = "NONE"
	if OrzUISetDB["totalGold"][realm] and OrzUISetDB["totalGold"][realm][name] then
		class = OrzUISetDB["totalGold"][realm][name][2]
	end
	return class
end
...
function G:FindProfleUser(icon)
	icon.list = {}
	for fullName, index in pairs(OrzUISetDB["ProfileIndex"]) do
...
function G:Note_OnEscape()
	self:SetText(OrzUISetDB["ProfileNames"][self.index])
end

function G:Note_OnEnter()
	local text = self:GetText()
	if text == "" then
		OrzUISetDB["ProfileNames"][self.index] = self.__defaultText
		self:SetText(self.__defaultText)
	else
		OrzUISetDB["ProfileNames"][self.index] = text
	end
end
...
function G:CreateProfileBar(parent, index)
	...
	if index == 1 then
		...
		M.PixelIcon(icon, "Interface\\Icons\\RaceChange", true) -- share
		icon.index = index
	...
	if not OrzUISetDB["ProfileNames"][index] then
		OrzUISetDB["ProfileNames"][index] = note.__defaultText
	end
	note:SetText(OrzUISetDB["ProfileNames"][index])
	note.index = index
	...
	local apply = G:CreateProfileIcon(bar, 2, "Atlas:common-icon-checkmark", U["SelectProfile"], U["SelectProfileTip"])
...
function G:Delete_OnEnter()
	...
	if OrzUISetDB["ProfileIndex"][text] or (OrzUISetDB["totalGold"][realm] and OrzUISetDB["totalGold"][realm][name]) then
		StaticPopup_Show("ORZUI_DELETE_UNIT_PROFILE", text, G:GetClassFromGoldInfo(name, realm))
	else
...
function G:CreateProfileGUI(parent)
	local reset = M.CreateButton(parent, 120, 24, U["OrzUI Reset"])
	reset:SetPoint("BOTTOMLEFT", 100, 60)
	reset:SetScript("OnClick", function()
		StaticPopup_Show("RESET_ORZUI")
	end)
	...
	restore:SetScript("OnClick", function()
		StaticPopup_Show("RESET_ORZUI_HELPINFO")
	end)]]
	...
	G.currentProfile = OrzUISetDB["ProfileIndex"][I.MyFullName]
...
function G:ExportGUIData()
	local text = "OrzUISettings:"..I.Version..":"..I.MyName..":"..I.MyClass
	...
	for KEY, VALUE in pairs(OrzUISetDB) do
		...
		elseif KEY == "ClickSets" then
			text = text..";ACCOUNT:"..KEY
			if OrzUISetDB[KEY][I.MyClass] then
				text = text..":"..I.MyClass
				for fullkey, value in pairs(OrzUISetDB[KEY][I.MyClass]) do
					value = gsub(value, "%:", "`")
					value = gsub(value, ";", "}")
...
function G:ImportGUIData()
	...
	if title ~= "OrzUISettings" then --or IsOldProfileVersion(version) 
	...
	for i = 2, #options do
		...
		if arg1 == "true" or arg1 == "false" then
			if key == "ACCOUNT" then
				OrzUISetDB[value] = toBoolean(arg1)
			else
		...
		elseif key == "ACCOUNT" then
			if spellBooleanValues[value] then
				local results = {select(3, strsplit(":", option))}
				for i = 1, #results, 2 do
					OrzUISetDB[value][tonumber(results[i])] = toBoolean(results[i+1])
				end
			elseif value == "RaidDebuffs" then
				local instName, spellID, priority = select(3, strsplit(":", option))
				if not OrzUISetDB[value][instName] then OrzUISetDB[value][instName] = {} end
				OrzUISetDB[value][instName][tonumber(spellID)] = tonumber(priority)
			elseif value == "CornerSpells" then
				...
				if class == I.MyClass then
					for i = 2, #results, 6 do
						...
						if not OrzUISetDB[value][class] then OrzUISetDB[value][class] = {} end
						if anchor == "" then
							OrzUISetDB[value][class][spellID] = {}
						else
							OrzUISetDB[value][class][spellID] = {anchor, {r, g, b}, filter}
						end
					end
				end
			elseif value == "ContactList" then
				...
				for i = 1, #names, 4 do
					OrzUISetDB[value][names[i]] = names[i+1]..":"..names[i+2]..":"..names[i+3]
				end
			elseif value == "ProfileIndex" then
				local results = {select(3, strsplit(":", option))}
				for i = 1, #results, 2 do
					OrzUISetDB[value][results[i]] = tonumber(results[i+1])
				end
			elseif value == "ProfileNames" then
				local results = {select(3, strsplit(":", option))}
				for i = 1, #results, 2 do
					OrzUISetDB[value][tonumber(results[i])] = results[i+1]
				end
			elseif value == "ClickSets" then
				if arg1 == I.MyClass then
					OrzUISetDB[value][arg1] = OrzUISetDB[value][arg1] or {}
					local results = {select(4, strsplit(":", option))}
					for i = 1, #results, 2 do
						results[i+1] = gsub(results[i+1], "`", ":")
						results[i+1] = gsub(results[i+1], "}", ";")
						OrzUISetDB[value][arg1][results[i]] = tonumber(results[i+1]) or results[i+1]
					end
				end
			end
...
function G:CreateDataFrame()
	...
	StaticPopupDialogs["ORZUI_IMPORT_DATA"] = {
	...
	accept:SetScript("OnClick", function(self)
		if self.text:GetText() ~= OKAY and dataFrame.editBox:GetText() ~= "" then
			StaticPopup_Show("ORZUI_IMPORT_DATA")
		end
		dataFrame:Hide()
	end)
