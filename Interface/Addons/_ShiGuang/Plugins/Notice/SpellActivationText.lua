local topid,topid2,leftid,leftid1,leftid2,rightid,rightid1,rightid2
...
local lefttext = AddTextFrame(height,width,"lefttext","RIGHT","LEFT",-15,0,30)
...
local righttext = AddTextFrame(height,width,"toptext","LEFT","RIGHT",15,0,30)
...
hooksecurefunc("SpellActivationOverlay_ShowAllOverlays", function(self, spellID, texture, positions, scale, r, g, b)

	--如果右边ID查不到,就把他设置成左边的spellID
	if not C_UnitAuras.GetPlayerAuraBySpellID(spellID) then
		if (positions == "Right (Flipped)" or positions == "Left + Right (Flipped)") and scale > 1 then
			Oldrightid = spellID
			spellID = leftid
		elseif (positions == "Right (Flipped)" or positions == "Left + Right (Flipped)") and scale == 1 then
			Oldrightid1 = spellID
			spellID = leftid1
		elseif (positions == "Right (Flipped)" or positions == "Left + Right (Flipped)") and scale < 1 then
			Oldrightid2 = spellID
	...
	if positions == "Top" and scale >= 1 then
	...
	if (positions == "Left" or positions == "Left + Right (Flipped)") and scale > 1 then
...
--替换不同层数的材质
hooksecurefunc("SpellActivationOverlay_ShowOverlay", function(self, spellID, texturePath, position, r, g, b)
	if not spellID then return end
	if not C_UnitAuras.GetPlayerAuraBySpellID(spellID) then return end
...
hooksecurefunc("SpellActivationOverlay_HideOverlays", function(self, spellID)
