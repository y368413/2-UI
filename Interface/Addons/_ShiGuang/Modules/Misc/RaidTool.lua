...
function MISC:RaidTool_CombatRes(parent)
	...
	res.Timer = M.CreateFS(frame, 16, "00:00", false, "RIGHT", -5, 0)
...
function MISC:RaidTool_BuffChecker(parent)
	...
	frame:HookScript("OnEnter", function(self)
		...
		if potionCheck then
			GameTooltip:AddDoubleLine(I.RightButton.."(Alt) "..I.InfoColor..U["ExRT Potioncheck"])
		end
		GameTooltip:Show()
	end)
...
function MISC:RaidTool_EasyMarker()
	...
	local menuList = {
		...
		{text = M.HexRGB(.98, .57, 0)..RAID_TARGET_2.." "..ICON_LIST[2].."12|t", func = function() SetRaidTarget("target", 2) end},
		{text = M.HexRGB(.83, .22, .9)..RAID_TARGET_3.." "..ICON_LIST[3].."12|t", func = function() SetRaidTarget("target", 3) end},
		{text = M.HexRGB(.04, .95, 0)..RAID_TARGET_4.." "..ICON_LIST[4].."12|t", func = function() SetRaidTarget("target", 4) end},
		{text = M.HexRGB(.7, .82, .875)..RAID_TARGET_5.." "..ICON_LIST[5].."12|t", func = function() SetRaidTarget("target", 5) end},
		{text = M.HexRGB(0, .71, 1)..RAID_TARGET_6.." "..ICON_LIST[6].."12|t", func = function() SetRaidTarget("target", 6) end},
		{text = M.HexRGB(1, .24, .168)..RAID_TARGET_7.." "..ICON_LIST[7].."12|t", func = function() SetRaidTarget("target", 7) end},
		{text = M.HexRGB(.98, .98, .98)..RAID_TARGET_8.." "..ICON_LIST[8].."12|t", func = function() SetRaidTarget("target", 8) end},
	}
