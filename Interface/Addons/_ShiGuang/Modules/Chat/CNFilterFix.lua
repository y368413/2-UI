...
local function _FriendsFrame_ShouldShowSummonButton(self)
	...
	if parent.buttonType == FRIENDS_BUTTON_TYPE_WOW then
	...
	elseif (parent.buttonType == FRIENDS_BUTTON_TYPE_BNET) then
...
local function travelPassButton_OnEnter(self)
	local parent = self:GetParent()
	local restriction = FriendsFrame_GetInviteRestriction(parent.id)
	if restriction == 11 then
		GameTooltip:SetOwner(self, "ANCHOR_RIGHT")
		local guid = FriendsFrame_GetPlayerGUIDFromIndex(parent.id)
...
function module:CNLanguageFilterFix()
	...
	if FriendsListFrameScrollFrame.buttons then
		for i = 1, #FriendsListFrameScrollFrame.buttons do
			FriendsListFrameScrollFrame.buttons[i]:HookScript("OnEnter", FriendTipOnEnter)
			FriendsListFrameScrollFrame.buttons[i].travelPassButton:HookScript("OnEnter", travelPassButton_OnEnter)
		end
