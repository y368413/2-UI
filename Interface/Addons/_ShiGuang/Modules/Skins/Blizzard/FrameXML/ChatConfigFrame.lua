...
tinsert(R.defaultThemes, function()
	...
	M.StripTextures(ChatConfigFrame)
	M.SetBD(ChatConfigFrame)
	M.StripTextures(ChatConfigFrame.Header)
	...
	hooksecurefunc("ChatConfig_UpdateCheckboxes", function(frame)
		...
		for index in ipairs(frame.checkBoxTable) do
			...
			if checkbox and not checkbox.styled then
				...
				local bg = M.CreateBDFrame(checkbox, .25)
				bg:SetInside()
				M.ReskinCheck(_G[checkBoxName.."Check"])
	...
	hooksecurefunc("ChatConfig_CreateTieredCheckboxes", function(frame, checkBoxTable)
		...
		for index, value in ipairs(checkBoxTable) do
			local checkBoxName = nameString..index
			M.ReskinCheck(_G[checkBoxName])

			if value.subTypes then
				for i in ipairs(value.subTypes) do
					M.ReskinCheck(_G[checkBoxName.."_"..i])
				end
			end
		end
	...
	hooksecurefunc("ChatConfig_UpdateSwatches", function(frame)
		...
		for index in ipairs(frame.swatchTable) do
			...
			if not bu.styled then
				M.StripTextures(bu)
				M.CreateBDFrame(bu, .25):SetInside()
				M.ReskinColorSwatch(_G[baseName.."ColorSwatch"])
				bu.styled = true
			end
		end
	end)

	local bg = M.CreateBDFrame(ChatConfigCombatSettingsFilters, .25)
	bg:SetPoint("TOPLEFT", 3, 0)
	bg:SetPoint("BOTTOMRIGHT", 0, 1)

	M.Reskin(CombatLogDefaultButton)
	M.Reskin(ChatConfigCombatSettingsFiltersCopyFilterButton)
	M.Reskin(ChatConfigCombatSettingsFiltersAddFilterButton)
	M.Reskin(ChatConfigCombatSettingsFiltersDeleteButton)
	M.Reskin(CombatConfigSettingsSaveButton)
	M.Reskin(ChatConfigFrameOkayButton)
	M.Reskin(ChatConfigFrameDefaultButton)
	M.Reskin(ChatConfigFrameRedockButton)
	M.ReskinArrow(ChatConfigMoveFilterUpButton, "up")
	M.ReskinArrow(ChatConfigMoveFilterDownButton, "down")
	M.ReskinInput(CombatConfigSettingsNameEditBox)
	M.ReskinRadio(CombatConfigColorsColorizeEntireLineBySource)
	M.ReskinRadio(CombatConfigColorsColorizeEntireLineByTarget)
	M.ReskinColorSwatch(CombatConfigColorsColorizeSpellNamesColorSwatch)
	M.ReskinColorSwatch(CombatConfigColorsColorizeDamageNumberColorSwatch)
	M.ReskinTrimScroll(ChatConfigCombatSettingsFilters.ScrollBar)
	...
	-- TextToSpeech
	M.StripTextures(TextToSpeechButton, 5)

	M.Reskin(TextToSpeechFramePlaySampleButton)
	M.Reskin(TextToSpeechFramePlaySampleAlternateButton)
	M.Reskin(TextToSpeechDefaultButton)
	M.ReskinCheck(TextToSpeechCharacterSpecificButton)

	M.ReskinDropDown(TextToSpeechFrameTtsVoiceDropdown)
	M.ReskinDropDown(TextToSpeechFrameTtsVoiceAlternateDropdown)
	M.ReskinSlider(TextToSpeechFrameAdjustRateSlider)
	M.ReskinSlider(TextToSpeechFrameAdjustVolumeSlider)
	...
	-- voice pickers
	M.StripTextures(ChatConfigTextToSpeechChannelSettingsLeft)
end)