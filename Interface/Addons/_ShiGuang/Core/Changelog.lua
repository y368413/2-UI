...
local UIQuestsandHelp = {
   ...
   "【重写】整个控制台彻底重写[界面如有不合理的地方可选择调整或直接重置]",
   "【重写】菜单栏，现在可以关闭和调整更多",
   "【重写】队伍工具，现在就位等信息全部集成到左上工具中",
   "【重写】标记工具，标记条可在控制台设置[Ctrl点击目标可快速标记]",
   "【修复】头像样式一焦点施法条导致全局载入失败的不过 [感谢@不要搞事]",
   "【修复】血条显示目标怪物是否精英和稀有的图标缺失问题 [感谢@河河]",
   "【调整】屏幕上方的位置信息可在控制台关闭",
   "【新增】糊涂大佬新写的 TankHappy 小插件",
   "【修复】快捷聊天条的一个小bug(感谢 赵赵 反馈)",
   "【移除】编辑模式微调插件，如需要，自行下载FineTuneHUD",
   "【移除】[职业]Buff监控(随机坐骑按钮)，如有需要的自行到官网网盘下载",
