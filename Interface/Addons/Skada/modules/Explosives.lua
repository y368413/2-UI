...
local function SwingDamage(timestamp, eventtype, srcGUID, srcName, srcFlags, dstGUID, dstName, dstFlags, ...)
    SpellDamage(timestamp, eventtype, srcGUID, srcName, srcFlags, dstGUID, dstName, dstFlags, 6603, C_Spell.GetSpellInfo(6603), "spellShcool", ...)
end
...
function mod:Update(win, set)
	...
	for i, player in ipairs(set.players) do
		if player.orb_hit > 0 or player.orb_click > 0 then
			...
			d.valuetext = "Tar:" .. Skada:FormatNumber(player.orb_click) .. " Hit:" .. Skada:FormatNumber(player.orb_hit)..(" (%02.1f%%)"):format(player.orb_hit / set.orb_count * 100)
			d.id = player.id
...
function mod:OnEnable()
	spellmod.metadata 		= {}
	mod.metadata 			= {click1 = spellmod, showspots = true, icon = 2175503 } --select(3, C_ChallengeMode.GetAffixInfo(13))
